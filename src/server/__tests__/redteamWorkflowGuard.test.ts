...
describe('redteam workflow guard', () => {
  ...
  it('injects coverage oracle bridge guidance for routed redteam runs', async () => {
    ...
    expect(confirmed.content).toContain('off_taxonomy_pass.md')
    expect(confirmed.content).toContain('8-15 target-specific attack hypotheses')
    expect(confirmed.content).toContain('taxonomy_escape_hatch')
    expect(confirmed.content).toContain('cloud_storage_bucket_object_boundary')
    expect(confirmed.content).toContain('bucket takeover')

...
6. Coverage oracle bridge is a hard workflow aid when the oracle is available:
   ...
   - Before checklist execution, run one off-taxonomy/freeform pass and write off_taxonomy_pass.md. It must list 8-15 target-specific attack hypotheses, at least three hypotheses not named by the default taxonomy, and any cloud/storage/IAM/CI/CD/client-side/agent-runtime surfaces that should be promoted into target-specific classes or method events. Reason from the target first; use the oracle to prevent blind spots after that.
   ...
   - Read oracle artifacts in this order when choosing the next action: off_taxonomy_pass.md, breakthrough_focus.md, taxonomy_gap_matrix.md, operator_action_router.md, operator_input_bundle_work_order.md, operator_execution_checklist.md, fingerprint_tool_lanes.md, discovery_execution_batches.md, candidate_proof_queue.md, candidate_seed_lineage.md, evidence_integrity.md.
   - Treat taxonomy_escape_hatch as the anti-checklist lane: if the freeform pass finds a strong target-specific path, keep it alive until packet evidence, a precise blocker, or an explicit scope decision closes it.
   - Treat cloud_storage_bucket_object_boundary as a first-class critical path, not a side effect of upload/media/leak checks. Cover bucket takeover, public read/write, object-key isolation, signed or presigned URL scope/lifetime, CDN origin drift, artifact bucket exposure, cross-tenant object access, and controlled object overwrite/delete with owned objects and replayable evidence.
...
7. Runtime and command safety are hard requirements, not suggestions:
   ...
   - Do not drop a high-impact path just because direct exploitation would require brute force, destructive writes, denial of service, persistence, high concurrency, or out-of-scope assets. Convert it into bounded proof, offline reproduction, self-owned test data validation, replayable negative proof, or an exact blocked-input/scope request; never fabricate evidence or impact.
